...
def calculate_kpis(order_data, website_sessions, website_pageviews):
    ...
    kpis["total_units_sold"] = order_data["order_item_id"].count()
    kpis["total_refunds"] = order_data["order_item_refund_id"].nunique()

    gross_revenue = order_data["price_usd_item"].sum()
    total_cogs = order_data["cogs_usd_item"].sum()
    ...
    try:
        revenue_per_channel = (
            ...
            .groupby("utm_source")["price_usd_item"]
            .sum()
            .reset_index()
            .rename(columns={"price_usd_item": "gross_revenue"})
