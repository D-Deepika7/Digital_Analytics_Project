...
# def line_chart_conversion_rate(order_data, website_sessions):
#     website_sessions['year_month'] = pd.to_datetime(website_sessions['created_at']).dt.to_period('M')
#     order_data['year_month'] = pd.to_datetime(order_data['created_at_session']).dt.to_period('M')
...
#     ax.plot(df['year_month'].astype(str), df['conversion_rate_pct'], marker='o', color='green')
#     ax.set_title("Conversion Rate (%) by Year and Month")
...
# def pie_chart_total_sessions(website_sessions):
#     data = website_sessions['utm_source'].fillna('Unknown')
#     data = data[data.str.upper() != 'NULL']
#     counts = data.value_counts()

#     fig, ax = plt.subplots()
#     ax.pie(counts, labels=counts.index, autopct='%1.1f%%', startangle=140, shadow=True)
#     ax.set_title("Total Sessions by UTM Source")
#     ax.axis('equal')
#     st.pyplot(fig)


# def bar_chart_gross_revenue(order_data):
#     df = order_data[order_data['utm_source'].notna()]
#     df = df[df['utm_source'].str.upper() != 'NULL']
#     revenue = df.groupby('utm_source')['price_usd_item'].sum().reset_index()
#     revenue = revenue.sort_values(by='price_usd_item', ascending=False)

#     fig, ax = plt.subplots(figsize=(10, 6))
#     bars = ax.bar(revenue['utm_source'], revenue['price_usd_item'], color='skyblue')
...
# def line_column_avg_time_by_session_path(website_pageviews):
#     # Ensure datetime
...
#     combined['path_short'] = combined['pageview_url'].apply(lambda x: x[:60] + '...' if len(x) > 60 else x)

#     # Aggregate by path
#     summary = (
#         combined.groupby('path_short')
...
#     # Bar: Average Session Duration
#     fig.add_trace(go.Bar(
#         x=summary['path_short'],
...
#         xaxis=dict(title='Session Path', tickangle=45),
#         yaxis=dict(title='Avg Session Duration (min)', side='left', showgrid=False),
...
def line_chart_total_orders_over_time(order_data):
    order_data['year_month'] = pd.to_datetime(order_data['created_at_order']).dt.to_period('M').astype(str)
    orders_by_month = (
...
def line_column_revenue_orders_by_product(order_data):
    df = order_data.copy()
    df['year_month'] = pd.to_datetime(df['created_at_order']).dt.to_period('M').astype(str)

    grouped = df.groupby(['year_month', 'product_name']).agg(
        gross_revenue=('price_usd_item', 'sum'),
...
def donut_units_sold_by_product(order_data):
    df = order_data.groupby('product_name')['order_item_id'].count().reset_index(name='units_sold')
    fig = px.pie(
        df,
        values='units_sold',
        names='product_name',
